...
class Recording:
    ...
    def from_file(path): # with name
        ...
        file_list = os.listdir(processed_image_folder)
        for filename in sorted(file_list, key = lambda x: int(x.split('.')[0])):
        ...
        file_list = os.listdir(odom_folder)
        for filename in sorted(file_list, key = lambda x: int(x.split('.')[0])):
        ...
        if os.path.exists(ground_truth_folder):
            file_list = os.listdir(ground_truth_folder)
            for filename in sorted(file_list, key = lambda x: int(x.split('.')[0])):
                if filename.endswith('.json'):
        ...
        if not valid:
            ...
            recording.to_file(path) # overwrite
            valid = True
    ...
    def to_file(self, path): # with name
        ...
        if len(self.ground_truths) > 0:
            ...
            for i, ground_truth in enumerate(self.ground_truths):
                with open(ground_truth_folder + '/' + str(i) + '.json', 'w') as f:
